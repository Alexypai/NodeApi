...
const {getAnime} = require ("./src/services/webhook/animeService")
const {getWeatherResponse} = require("./src/services/webhook/weatherService");
    ...
    sessionClient.detectIntent(request).then(async (responses) => {
        ...
        switch (true) {
            case (result.intent.displayName.includes("weather")):
                ...
                break;
            case (result.intent.displayName === "get-anime-details"):
                let animeDetails = '';
                if(result.allRequiredParamsPresent === true) {
                    let animeDetails = await getAnime();
                }
                let title = animeDetails.title;
                let synopsis = animeDetails.synopsis;

                chatBotResult = result.fulfillmentText + title + ' et voici son synopsis : ' + synopsis;
